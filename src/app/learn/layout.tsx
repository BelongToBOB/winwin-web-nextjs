...
interface CourseContext { slug: string; title: string; chapters: CurriculumChapter[]; }
...
function SidebarContent({ onClose }: { onClose?: () => void }) {
  ...
  useEffect(() => {
    if (!isInsideCourse || !session?.user?.email) { setCourseCtx(null); return; }
    fetch(`${LMS_API}/learn/courses/${currentSlug}`, { headers: { "x-user-email": session.user.email } })
      .then(r => r.ok ? r.json() : null)
      .then(d => d ? setCourseCtx({ slug: d.slug, title: d.title, chapters: d.chapters }) : setCourseCtx(null))
    ...
    <CourseCtx.Provider value={courseCtx}>
      ...
      <nav className="flex-1 overflow-y-auto">
        {isInsideCourse && courseCtx ? (
          <div className="p-3">
            <Link href="/learn" onClick={onClose} className="flex items-center gap-2 rounded-lg px-3 py-2 text-xs text-gray-500 hover:bg-white/5 hover:text-gray-300">
            ...
            <div className="mt-3 mb-4 px-3"><h3 className="text-sm font-semibold text-white leading-snug">{courseCtx.title}</h3></div>
            <div className="space-y-4">
              {courseCtx.chapters.map(ch => (
                <div key={ch.id}>
                  <div className="flex items-center justify-between px-3 mb-1">
                    <span className="text-[11px] font-medium uppercase tracking-wider text-gray-600 truncate">{ch.title}</span>
                    <span className="text-[10px] tabular-nums text-gray-600 shrink-0 ml-2">{ch.lessons.filter(l => l.completed).length}/{ch.lessons.length}</span>
                  </div>
                    ...
                    {ch.lessons.map(lesson => {
                      ...
                      return (
                        <Link key={lesson.id} href={`/learn/${currentSlug}/${lesson.id}`} onClick={onClose}
                          className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] transition ${active ? "bg-yellow-accent/10 text-yellow-accent" : "text-gray-400 hover:bg-white/5 hover:text-gray-200"}`}>
                          <div className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${lesson.completed ? "border-yellow-accent/40 bg-yellow-accent/10" : active ? "border-yellow-accent/30" : "border-white/10"}`}>
                            {lesson.completed ? (
                              <svg className="h-3 w-3 text-yellow-accent" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                            ) : (
                              <span className={`text-[9px] ${active ? "text-yellow-accent" : "text-gray-600"}`}>{lesson.type === "text" ? "T" : lesson.type === "file" ? "F" : "V"}</span>
                            )}
                          </div>
                          <span className="flex-1 truncate">{lesson.title}</span>
                          <span className="text-[10px] tabular-nums text-gray-600 shrink-0">{formatDur(lesson.duration)}</span>
                        </Link>
                      );
                    })}
                  </div>
          ...
          <div className="p-3 space-y-1">
            <Link href="/learn" onClick={onClose} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm transition ${isLearnHome ? "bg-white/10 text-white font-medium" : "text-gray-400 hover:bg-white/5 hover:text-gray-200"}`}>
            ...
            {courses.length > 0 && <div className="pt-3 pb-1 px-3"><span className="text-[11px] font-medium uppercase tracking-wider text-gray-600">คอร์สของฉัน</span></div>}
            {courses.map(course => (
              <Link key={course.id} href={`/learn/${course.slug}`} onClick={onClose}
                className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition ${currentSlug === course.slug ? "bg-white/10 text-white font-medium" : "text-gray-400 hover:bg-white/5 hover:text-gray-200"}`}>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    <div className="h-1 flex-1 rounded-full bg-white/10 overflow-hidden"><div className="h-full rounded-full bg-yellow-accent" style={{ width: `${course.progress.percent}%` }} /></div>
      ...
      {session?.user && (
        <div className="border-t border-white/10 p-3 shrink-0">
          {(session.user as any).role === "admin" && (
            <Link href="/admin/courses" onClick={onClose} className="flex items-center gap-2 rounded-lg px-3 py-2 mb-1 text-xs text-gray-500 hover:bg-white/5 hover:text-gray-300">
...
function LearnShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-black text-gray-50">
      {/* Mobile: top bar + content stacked vertically */}
      <div className="lg:hidden">
        {/* Fixed top bar */}
        <div className="sticky top-0 z-40 flex h-14 items-center justify-between border-b border-white/10 bg-black/90 backdrop-blur-md px-4">
        ...
        {mobileOpen && (
          <div className="fixed inset-0 z-50" onClick={() => setMobileOpen(false)}>
            ...
            <aside className="absolute left-0 top-0 bottom-0 flex w-[280px] max-w-[85vw] flex-col bg-[#0A0A0A] border-r border-white/10" onClick={e => e.stopPropagation()}>
      ...
      <div className="hidden lg:flex lg:h-screen">
        <aside className="flex w-72 shrink-0 flex-col border-r border-white/10 bg-[#0A0A0A] overflow-y-auto">
          <SidebarContent />
        </aside>
