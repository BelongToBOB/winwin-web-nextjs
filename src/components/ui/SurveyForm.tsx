...
const SKILL_LEVELS = [
  ...
  { value: "advanced", label: "ระดับเชี่ยวชาญ", desc: "เข้าใจโครงสร้างดี แต่อยากได้แนวทางเชิงลึก" },
];
...
export default function SurveyForm({ apiBase = "https://checkout.winwinwealth.co/api" }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    if (needsReceipt) {
      ...
      if (!receiptTaxId?.trim() || !/^\d{13}$/.test(receiptTaxId.trim())) { alert("กรุณากรอกเลขประจำตัวผู้เสียภาษี 13 หลัก"); return; }
      if (!receiptEmail?.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(receiptEmail.trim())) { alert("กรุณากรอกอีเมลรับใบเสร็จที่ถูกต้อง"); return; }
    }
    if (needsWithholding) {
